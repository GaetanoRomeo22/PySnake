...
# Immagini
player1_icon = pygame.image.load('snake_head.png')
player1_icon = pygame.transform.scale(player1_icon, (SNAKE_SIZE, SNAKE_SIZE))
...
def draw_snake(snake_pos, head_icon, body_icon):
    ...
    screen.blit(head_icon, (snake_pos[0][0], snake_pos[0][1]))

    # Disegna il corpo del serpente
    for pos in snake_pos[1:]:
        screen.blit(body_icon, (pos[0], pos[1]))
...
def game(fps, mode):
    ...
    while running:
        ...
        # Disegna il serpente
        draw_snake(snake1_pos, player1_icon, snake_body)

        if mode == "Multiplayer":
            draw_snake(snake2_pos, player1_icon, snake_body)
